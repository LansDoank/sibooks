<x-dashboard :title="$title" :heading="$heading" :user="$user" >
    <section class="bg-white dark:bg-gray-900  border shadow rounded">
        <div class="py-4 px-4 mx-auto max-w-6xl lg:py-16">
            <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">Edit Akun Siswa</h2>
            <form action="/admin/user/update" method="post">
                ...
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    ...
                    <div class="w-full">
                        ...
                        <input value="{{ $account->email }}" type="email" name="email" id="email"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                    ...
                    <div>
                        ...
                        <select id="category" name="role" 
                            ...
                            <option value="{{ $role->id }}" {{ $role->id == $account->id ? "selected" : "" }}>{{ $role->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
